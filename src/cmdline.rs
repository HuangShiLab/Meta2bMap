...
#[derive(Parser)]
#[clap(author, version, about = "Coverage-adjusted ANI queries and taxonomic profiling for 2bRAD and shotgun metagenomic data.\n\n--- Preparing inputs by extracting (indexing) 2bRAD tags\n## genomes and reads together; outputs <name>.syldb / <name>.sylsp in the -d directory\nmeta2bmap extract -t 5 -g genome1.fa genome2.fa -r sample1.fq -o out -d out -n run1 -e BcgI\n\n## multi-enzyme\nmeta2bmap extract -t 5 -g genome1.fa -r sample1.fq -o out -d out -n run1 -e BcgI,BslFI,CspCI,AloI\n\n--- Nearest neighbour coverage-adjusted ANI\nmeta2bmap query --db-file out/run1.syldb --sample-file out/run1.sylsp > query.tsv\n\n--- Taxonomic profiling with relative abundances and ANI\nmeta2bmap profile --db-file out/run1.syldb --sample-file out/run1.sylsp --minimum-ani 90 --tsv-name abund.tsv --threads 5 > profile.tsv", arg_required_else_help = true, disable_help_subcommand = true)]
pub struct Cli {
    #[clap(subcommand,)]
    pub mode: Mode,
}
...
pub struct ProfileArgs {
    ...
    #[arg(
        ...
        help = "Enable winner-take-all over-stripping protection (default: OFF). When ON, a genome with >=100 initially shared tags that keeps <=5% of them after reassignment (near-total annihilation) and would fall below the reporting floor is kept with its pre-reassignment ANI/tag counts; its abundance still uses post-reassignment coverage so reads are not double-counted. NOTE: on large dense databases this rescues many strain-cluster losers alongside true positives (precision cost measured on CAMI2); OFF is the safer default."
    )]
    pub reassign_protection: bool,
}
